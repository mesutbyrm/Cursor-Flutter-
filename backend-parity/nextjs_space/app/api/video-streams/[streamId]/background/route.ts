...
async function patchStream(req: NextRequest, context: Ctx) {
  ...
  const forwarded = new NextRequest(req.url, {
    ...
    body: JSON.stringify({
      backgroundUrl:
        (body as { backgroundUrl?: string }).backgroundUrl ??
...
export async function POST(req: NextRequest, context: Ctx) {
  try {
    ...
    console.error('[video-streams/[streamId]/background POST]', e)
    return NextResponse.json({ error: 'Arka plan güncellenemedi' }, { status: 500 })
  }
}
...
export async function PATCH(req: NextRequest, context: Ctx) {
  try {
    ...
    console.error('[video-streams/[streamId]/background PATCH]', e)
    return NextResponse.json({ error: 'Arka plan güncellenemedi' }, { status: 500 })
  }
}
