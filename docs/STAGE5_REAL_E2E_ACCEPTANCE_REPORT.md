...
| PK | **PASS (API 2-user)** / BLOCKED (live) | create→accept→end API doğrulandı |
| MUSIC | **PASS (paid request API)** / BLOCKED (playback) | jeton düşümü OK; adb ses yok |
| SSE | PASS | room stream + gift event |
| TRTC | **PASS (token API)** / BLOCKED (SDK) | A/B token OK; adb yok |
...
| PK | Voice 2-user accept | ✅ PASS | api-stage5-unblock: create→accept→end |
| PK | Live PK | ⏸️ BLOCKED | onaylı LIVE host gerekli |
| TRTC | Backend token A/B | ✅ PASS | api-stage5-unblock |
...
2. ~~**PK 2-user**~~ — `api-stage5-unblock.sh` ile create→accept→end **PASS**.
3. ~~**TRTC token**~~ — backend token A/B **PASS** (yanıt `data.userSig` içinde).
4. ~~**Gift SSE**~~ — network stream'de gift event **PASS**.
5. **LIVE host:** `cursor.host.*` hesabı `NOT_APPROVED` (teller başvurusu pending) — **sizin admin onayınız gerekli**.
6. **TEST_PSYCHIC:** `ACCEPTANCE_TELLER_*` secret yok — accept akışı test edilemedi.
7. **Gerçek cihaz:** `adb devices` boş — TRTC SDK enterRoom, ses, animasyon doğrulanamadı.
8. **0-jeton testi:** overspend ile yetersiz bakiye **PASS** (sıfır bakiye hesabı gerekmez).

Kullanıcı kılavuzu: `docs/KULLANICI_TEST_KILAVUZU.md`  
Tek komut: `bash scripts/run-tests-for-user.sh`  
Unblock betiği: `scripts/acceptance-tests/api-stage5-unblock.sh`
