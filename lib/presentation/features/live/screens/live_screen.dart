...
class LiveWatchScreen extends ConsumerWidget {
  ...
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    return FutureBuilder<SocialRepository>(
      future: ref.watch(socialRepositoryProvider.future),
      builder:
          (BuildContext context, AsyncSnapshot<SocialRepository> snapshot) {
            if (!snapshot.hasData) {
          ...
          child: SafeArea(
            child: Row(
              children: <Widget>[
                ...
                FilledButton.tonal(
                  onPressed: () =>
                      ref.read(authControllerProvider).followProfile(stream.host),
