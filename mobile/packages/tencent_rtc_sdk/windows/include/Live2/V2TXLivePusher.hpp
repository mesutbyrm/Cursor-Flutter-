...
class V2TXLivePusher {
   public:
     ...
     *         - `path` is empty by default. No other values are defined (macOS, Android).
     * @note This API works only on Windows, macOS, and Android.
     */
     ...
     * @param config Please see the description of {@link V2TXLiveTranscodingConfig} in `V2TXLiveDef.hpp`. Passing in `nullptr` will cancel On-Cloud MixTranscoding.
     * @return Return code for {@link V2TXLiveCode}.
     *         - `V2TXLIVE_OK`: successful.
     ...
     * - If you are still in the room but do not need to mix streams anymore, make sure that you pass in `nullptr` to cancel On-Cloud MixTranscoding. The On-Cloud MixTranscoding module starts working the moment you enable On-Cloud MixTranscoding. You
     * may incur additional costs if you do not cancel it in a timely manner.
